  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <Modal visible={showCreateModal} transparent animationType="slide" onRequestClose={() => setShowCreateModal(false)}>
        <View style={styles.modalOverlay}><View style={styles.modal}><Text style={styles.modalTitle}>Create staff admin</Text><Text style={styles.modalSub}>This creates an operational staff account, not another Super Admin account.</Text><TextInput value={form.username} onChangeText={(username) => setForm((current) => ({ ...current, username }))} placeholder="Staff full name" autoCorrect={false} style={styles.input} /><TextInput value={form.email} onChangeText={(email) => setForm((current) => ({ ...current, email }))} placeholder="Staff email" autoCapitalize="none" autoCorrect={false} keyboardType="email-address" style={styles.input} /><TextInput value={form.password} onChangeText={(password) => setForm((current) => ({ ...current, password }))} placeholder="Temporary password (12+ characters)" autoCapitalize="none" autoCorrect={false} secureTextEntry style={styles.input} /><View style={styles.modalActions}><TouchableOpacity disabled={creating} onPress={() => setShowCreateModal(false)} style={styles.cancel}><Text>Cancel</Text></TouchableOpacity><TouchableOpacity disabled={creating} onPress={() => void createStaffAdmin()} style={[styles.create, creating && styles.disabled]}><Text style={styles.actionText}>{creating ? 'Creating...' : 'Create admin'}</Text></TouchableOpacity></View></View></View>
      </Modal>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modal}>
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity disabled={Boolean(processingId)} onPress={() => setBlotterDecision(null)} style={styles.cancel}><Text>Cancel</Text></TouchableOpacity>
